...
<h3>Indexer Options:</h3>
		{{ Form::open(array('action'=>'IndexersController@store', 'method'=>'post')) }}
			<div class='form-group'>
				<span class='glyphicon glyphicon-question-sign redish' data-toggle="popover" title="name" data-content="Enforced memory usage limit that the indexer 
				...
				the database server can timeout. You can resolve that either by raising timeouts on SQL server side or by lowering mem_limit."></span>
			...
			<div class='form-group'>
				<span class='glyphicon glyphicon-question-sign redish' data-toggle="popover" title="name" data-content="I/O throttling related option. 
				It limits maximum count of I/O operations (reads or writes) per any given second. A value of 0 means that no limit is imposed.
					...
					of that can help reduce search performance degradation caused by indexing."></span>
			...
			<div class='form-group'>
				<span class='glyphicon glyphicon-question-sign redish' data-toggle="popover" title="name" data-content="I/O throttling related option. It limits maximum 
				...
				all I/O calls should be under 256 KB (default internal buffer size) anyway, so max_iosize values higher than 256 KB must not affect anything."></span>
			...
			<div class='form-group'>
				<span class='glyphicon glyphicon-question-sign redish' data-toggle="popover" title="name" data-content="Write buffers are used to write both temporary and final index 
				...
				(currently up to 4) buffers for different files will be allocated, proportionally increasing the RAM usage."></span>
			...
			<div class='form-group'>
				<span class='glyphicon glyphicon-question-sign redish' data-toggle="popover" title="name" data-content="File field buffer is used to load files referred to 
				from sql_file_field columns. This buffer is adaptive, starting at 1 MB at first allocation, and growing in 2x steps until either file contents can be loaded,
				 or maximum buffer size, specified by max_file_field_buffer directive, is reached.<br />Thus, if there are no file fields are specified, no buffer is allocated
				  at all. If all files loaded during indexing are under (for example) 2 MB in size, but max_file_field_buffer value is 128 MB, peak buffer usage would still be
				   only 2 MB. However, files over 128 MB would be entirely skipped."></span>
			...
			<div class='form-group'>
				<span class='glyphicon glyphicon-question-sign redish' data-toggle="popover" title="name" data-content="When there is a problem indexing a file referenced 
				...
				document, or fail indexing entirely. on_file_field_error directive controls that behavior. The values it takes are:
					...
					doing actual parsing work. So in case a file goes away between these two open attempts, the document will still be indexed."></span>
			...
			<div class='form-group'>
				...
				<span class='glyphicon glyphicon-question-sign redish' data-toggle="popover" title="name" data-content="Our lemmatizer implementation (see Section 12.2.6, 
				...
				MB is needed, it will only use those 110 MB."></span>
